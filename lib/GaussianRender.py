...
def pts2render(data, bg_color):
    ...
    for i in range(bs):
        ...
        pts_xyz_i = torch.cat(xyz_i_valid, dim=0)
        pts_rgb_i = torch.cat(rgb_i_valid, dim=0)
        pts_rgb_i = pts_rgb_i * 0.5 + 0.5
        rot_i = torch.cat(rot_i_valid, dim=0)
        scale_i = torch.cat(scale_i_valid, dim=0)
        opacity_i = torch.cat(opacity_i_valid, dim=0)

        render_novel_i = render(data, i, pts_xyz_i, pts_rgb_i, rot_i, scale_i, opacity_i, bg_color=bg_color)

        # ✅ If render() returns a tuple, grab the first element
        if isinstance(render_novel_i, tuple):
            render_novel_i = render_novel_i[0]

        render_novel_list.append(render_novel_i.unsqueeze(0))

    data['novel_view']['img_pred'] = torch.cat(render_novel_list, dim=0)
    return data
